...
//**********************************************************************/
// Try to find substring "viewAttr№" in 'inputStr' and replace it with item from  "data" object
function replaceViewParamAttr2(inputStr, data) {
   var newString = inputStr;
   var ms = inputStr.match(/viewAttr(\d+)/g);
   if (ms) {
      ms.forEach(function (m) {
          if (m in data){
            newString = newString.replace(/viewAttr(\d+)/, data[m]);
          }
      });
  }
...
//**********************************************************************/
//Check 'inputStr' for "ViewAttr" substrings  and return converted one (return realTagID)   
function checkForViewAttr2(inputStr, widgetData) {

   if (inputStr && (inputStr.indexOf('viewAttr') >= 0)){
      inputStr = replaceViewParamAttr2(inputStr, widgetData); //replace "viewAttr"
    }
