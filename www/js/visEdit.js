...
vis = $.extend(true, vis, {
    ...
    editApplyPosition:      function (viewDiv, view, wid, top, left) {
        ...
        let  LeftPrefix = getAttributSufix(oldL);
        ...
        if (left !== null) {
            ...
            this.views[view].widgets[wid].style.left = left + LeftPrefix;
        }
        if (top  !== null) {
            if (!posT) {
            ...
            this.views[view].widgets[wid].style.top = top + TopPrefix;
        }
    },
    editApplySize:          function (viewDiv, view, wid, width, height) {
        ...
        let WidthPrefix = getAttributSufix(oldW);
        let HeightPrefix = getAttributSufix(oldH);
        ...
        if (width !== null) {
            ...
            this.views[view].widgets[wid].style.width  = width + WidthPrefix;
        }
        if (height !== null) {
            if (!posH && height.toString().indexOf('px') === -1) height += 'px';
            this.views[view].widgets[wid].style.height = height + HeightPrefix;
        }
    },
    dragging:               false,
    ...
    UpdateEditorActiveViewZoom:      function(updateSlider=true){
        ...
        let zoomValue = 1;
        let viewName = "no selected view";
        
        if ( this.activeView){
            viewName = this.activeView;
        ...
        $('#activeViewName').text(this.activeView);
    ...
    addtoViewtoHistory(viewName){
        if (this.viewNavigateHistory.length==0){
           this.viewNavigateHistory = new Array(10);
        }

        this.viewNavigatePos++;
        if (this.viewNavigatePos == this.viewNavigateHistory.length)
            this.viewNavigatePos=0;

        this.viewNavigateHistory[this.viewNavigatePos] = viewName;
    },
    
    _navigateViewHistindex(selectPrevView){
        let i;
        if (selectPrevView){
            i = this.viewNavigatePos - 1;
            if (i < 0) i = (this.viewNavigateHistory.length-1);
        }
        else{
            i = this.viewNavigatePos + 1;
            if (i == this.viewNavigateHistory.length)  i = 0;
        } 
        return i;
    },

    navigateViewHist(selectPrevView){
        let i =  this._navigateViewHistindex(selectPrevView);
        let viewName = this.viewNavigateHistory[i];   
...
//************************************************************************************ */
function selectPrevView(){
    vis.navigateViewHist(true);
    //$(this).attr('title', 'your new title');
}
function selectNextView(){
    vis.navigateViewHist(false);
}
...
$( document ).ready(function() {
    ...
    $("#goPrevView").click(selectPrevView);
    $("#goNextView").click(selectNextView);
});
